...
The gateway's `--controller-sync`/`--observe` and the relay's `--controller`
accept `host:port` with a DNS hostname (DDNS controllers), re-resolved on
every Sync reconnect (and, gateway only, every observe tick); dial targets
are IPv4-only (v1) — IPv6 and malformed IPv4-shaped literals are rejected at
boot with a non-zero exit. The shared pieces (bounded resolver, boot-time
`validate_host_port`, canonical keepalive constants) live in
`wiremesh-enroll`, re-exported under the original gateway/relay paths. The
gateway and relay Sync channels run HTTP/2 keepalive (15s/10s, while-idle)
+ a 10s connect timeout, and the controller's Sync listener mirrors the same
15s/10s server-side (all three derive from the one canonical constant set)
— fixes a live-found silently-half-open Watch stream (see
`docs/research/ops-finding-sync-half-open-stream.md`); remaining follow-ups
