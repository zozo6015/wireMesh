...
=== TEARDOWN STEP 1 (epoch-aware enforcer): DONE (commits 2016ef4 test, 7e4123b impl, opus review clean) ===
Single Arc<Mutex<GatewayEnforcer>> → shared Arc<Mutex<HashMap<u32,GatewayEnforcer>>> (keyed by epoch, seeded {0:wg0}); apply_state applies the current policy to EVERY live enforcer (per-version idempotent — a v1-created rotation enforcer DOES receive a later v2 update, traced); rotation bring-up inserts by epoch (fail-closed ordering preserved); metrics aggregate deny counters across all (fold = identity for the single-entry no-rotation case). maybe_start_role_b became async. CLOSES the "policy-tighten-post-cutover doesn't reach the active tun" security gap — new test policy_tighten_after_rotation_reaches_active_tun green. FULL non-regression: lib 76/76, key_rotation 3/3, mesh_milestone (default_deny scrape), nat 4/4, relay 2/2.
NEXT: Step 2 (epoch-aware DEVICE — unify epoch-0 into TunnelSet; apply_state peer-apply + set_peer_endpoint + change-guard target the active epoch's tun, driven by active_epoch=0-in-steady-state so nat/relay/mesh are non-regressive by construction) = the enabler for Step 3 (local-liveness retire trigger + tear down old Device + evict enforcer + done-bar asserting wg0 gone). Step 2 touches the 4b/4c punch/relay set_peer_endpoint path — the deepest/riskiest increment.

=== TEARDOWN STEP 2 (epoch-aware DEVICE) — ARCHITECTURAL OBSTACLE (scoped, NOT yet implemented) ===
Step 2 must make apply_state's PEER-APPLY and the punch/relay `set_peer_endpoint` (main.rs) target the ACTIVE epoch's tun, not hardcoded wg0. Obstacle: `set_peer_endpoint` runs in Send punch/relay tasks via `PathCtx` (Clone, holds `ifname`/`priv_key`/`wg_port`/`applied_wg0` as fixed wg0 values), but the active Device's private key lives in the NON-Send `TunnelSet` owned by the block_on'd run task — PathCtx can't hold TunnelSet. Fix design: PathCtx gains `Arc<Mutex<ActiveTunInfo{ ifname, priv_key: String, wg_port, applied_config_cache }>>` (all Send), seeded to wg0, UPDATED at cutover to wg0e<N>'s values; both `apply_state` and `set_peer_endpoint` resolve the active tun from it; the change-guard `applied_wg0: Option<String>` becomes per-active-tun (lives in ActiveTunInfo). apply_state's `tunnel` param → the active Device (tunnels.get(active_epoch)). NON-REGRESSION IS BY CONSTRUCTION: every non-rotation test (mesh/nat/relay) keeps active_epoch=0 → ActiveTunInfo == wg0 → byte-identical behavior; only a post-cutover state applies to wg0e<N>. RISK: touches the 4b/4c punch/relay set_peer_endpoint path → nat_matrix/relay_matrix are the gate. This is the DEEPEST increment (architectural PathCtx refactor); do it in a FRESH context to protect the multi-cycle non-regression. THEN Step 3 = local-liveness retire trigger (Role A tears down old Device once all peers rx-corroborated live on the new tun + grace) → tunnels.tear_down(old) + enforcers.remove(old) → done-bar asserting wg0 gone + mesh still works.

=== TEARDOWN STEP 2/3 (epoch-aware device + old-epoch teardown): DONE (commits e59dc03 test, a538967 impl, opus review SPEC ✅) ===
apply_state/set_peer_endpoint/run_path_ticks now resolve the tun from a shared ActiveTunInfo{ifname,priv_key,wg_port,change-guard} (absorbed applied_wg0); epoch-0 boots into TunnelSet; at cutover `active` flips to wg0e<N>; once every peer is rx-corroborated live on the new tun for RETIRE_GRACE(=2*ROTATION_KEEPALIVE), Role A tears down the OLD Device (tunnels.tear_down drops the old key from memory + enforcer evict) via a retire_ready shared-flag consumed by the run task. Make-before-break preserved (old epoch only, only post-cutover+grace). Impl found+fixed 2 subtle bugs: cutover seeds the change-guard so an unchanged config is a no-op on the new tun (avoids session-resetting re-apply); punch socket binds base_wg_port (not offset) so it doesn't steal the live tun's datagrams. NON-REGRESSION BY CONSTRUCTION (non-rotating gw never flips active): FULL green — lib 76, key_rotation 4/4 (incl. new old_epoch_device_is_torn_down_after_rotation), mesh, nat 4/4, relay 2/2.
KNOWN LIMITATIONS (docs/research/key-rotation-teardown-notes.md, all untested-scenario, none a regression):
  A (TOP must-fix): post-cutover DEVICE churn (peer CIDR/endpoint change, punch/relay) recomputes device_config_pinned = BASE-port endpoints → clobbers the live OFFSET-port session → black-hole. Seed masks it for the unchanged config only. FIX: thread offset-port rewrite through the active-tun apply path + a post-cutover-change test. Post-cutover device churn UNSUPPORTED until then.
  B: post-rotation NAT re-punch binds base port (live is offset) → can't restore direct; relay is fallback. Fix with A.
  C: retirement is PROCESS-LOCAL — EpochKeys::promote/retire never called; boot always loads epoch-0 identity key → rotation+reboot resurrects the retired key as the live device. Security goal met for the RUNNING process (key dropped from live Device before ip link del), NOT durable. Fix = wire rotation persistence (promote/retire store + boot-identity swap).
  D (Minor): Role-B post-cutover CIDR churn routes via wg0 not the overlap tun (narrow). Fold into multi-peer overlap.
