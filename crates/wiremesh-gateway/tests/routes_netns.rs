...
fn programs_mtu_forward_route_and_mss() {
    ...
    ns.exec(&["ip", "link", "add", "dum0", "type", "dummy"])
        .unwrap();
    ...
    let route = std::process::Command::new("ip")
        .args(["route", "show", "10.10.2.0/24"])
        .output()
        .unwrap();
    assert!(
        String::from_utf8_lossy(&route.stdout).contains("dum0"),
        "route present"
    );
    let fwd = std::process::Command::new("sysctl")
        .args(["-n", "net.ipv4.ip_forward"])
        .output()
        .unwrap();
    assert_eq!(String::from_utf8_lossy(&fwd.stdout).trim(), "1");
    let nft = std::process::Command::new("nft")
        .args(["list", "table", "inet", "wiremesh_mss"])
        .output()
        .unwrap();
    assert!(
        String::from_utf8_lossy(&nft.stdout).contains("maxseg"),
        "mss rule present"
    );
