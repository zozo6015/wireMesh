...
fn own_pubkey_hex_via_uapi(ifname: &str) -> Option<String> {
    ...
    stream.read_to_string(&mut resp).expect("reading UAPI get response");
...
fn two_epoch_tunnels_coexist_and_tear_down() {
    ...
    let own0_hex = own_pubkey_hex_via_uapi("wge0").expect("wge0 must report own_public_key via UAPI get=1");
    let own1_hex = own_pubkey_hex_via_uapi("wge1").expect("wge1 must report own_public_key via UAPI get=1");
    assert_eq!(own0_hex.len(), 64, "own_public_key must be 64 hex chars: {own0_hex}");
    assert_eq!(own1_hex.len(), 64, "own_public_key must be 64 hex chars: {own1_hex}");
    assert_eq!(own0_hex, k0_pub_hex, "wge0's UAPI own_public_key doesn't match the pubkey derived from its private key");
    assert_eq!(own1_hex, k1_pub_hex, "wge1's UAPI own_public_key doesn't match the pubkey derived from its private key");
    assert_ne!(own0_hex, own1_hex, "the two epochs' Devices must hold distinct own keys");
