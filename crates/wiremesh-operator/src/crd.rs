...
pub struct WiremeshControllerSpec {
    ...
    /// disable rotation by omission — the operator emits `off` explicitly, so
    /// the rendered Deployment STATES that no rotation timer runs instead of
    /// implying it, and so the manifest stays correct against a controller old
    /// enough to still read an absent variable as "rotate every 30 days" (the
    /// controller's own default flipped to no-timer on 2026-08-12; automatic
    /// rotation is opt-in there too now). Set (`Some`) is passed through
    /// verbatim; grammar validation (`off`/`<n>d`/`<n>h`/...) lives on the
    /// controller boot path, not here. Leaving it unset remains the
    /// recommendation: the in-step rotation done-bar passed in v0.9.1, but the
    /// rotation wedge (`docs/BACKLOG.md` item 9 — one part-failed rotation and
    /// a gateway silently ignores every later directive until it restarts) is
    /// still open, so arming the timer is a choice an operator makes here,
    /// deliberately. The operator owns this env key under SSA force-apply
    /// (`controllers::apply`/`apply_deployment`, `.force()` on both), so a
    /// hand-set value on the Deployment IS reconciled back to match this
    /// field on every pass — this field is the one place the interval is
    /// declared, not a bug to route around.
    #[serde(default, skip_serializing_if = "Option::is_none")]
