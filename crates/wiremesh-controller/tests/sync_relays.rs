...
//!
//! 1. `build_snapshot` must populate `StateSnapshot.relays` with a
...
async fn relay_enrollment_pushes_relays_delta_to_connected_gateway() {
    ...
    assert_eq!(
        delta.relays[0].endpoint, endpoint,
        "the delta's relay entry must carry the newly enrolled relay's endpoint"
    );
    assert_eq!(
        delta.relays[0].relay_id, relay_id as u64,
