//! Controller broker for simultaneous hole punching (spec §4). Accepts exactly
//! two TCP registrations `{"id","candidates":[...],"pubkey":...}`, then relays
//! each peer's candidates+pubkey to the other and fires a near-simultaneous "go"
//! by writing both replies back-to-back on one thread (µs-scale skew — the
//! primary go-skew guarantee, spec §4).
...
fn main() -> anyhow::Result<()> {
    ...
    // Each side is told about the OTHER peer. Back-to-back writes = tight go.
