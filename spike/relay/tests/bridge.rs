...
//
// Proves Bet 3's core claims (spec §6.1) for Task 13's relay:
...
//
// `relay::Client::connect_no_cert` does NOT expose that raw future, though —
// `finish_connect` wraps connect + open_bi (the registration stream) +
// write + ack-read into a single `Result`-returning async fn. Confirmed by
// running this test with the error printed (see the `eprintln!`s below):
// the `Err`'s message is
...
async fn bridges_datagrams_and_rejects_certless_clients() {
    ...
    // --- Property 1: bridging ---------------------------------------
    let a = relay::Client::connect(relay_addr, &dir, "gw-A")
        .await
        .expect("gw-A connect+register");
    let b = relay::Client::connect(relay_addr, &dir, "gw-B")
        .await
    ...
    let no_cert_result = relay::Client::connect_no_cert(relay_addr, &dir).await;
