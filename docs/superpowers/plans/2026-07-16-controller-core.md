...
- **Build/test environment:** host **stable Rust** — Cycle 2 is pure userspace (no tun/eBPF/netns), so no privileged container. `cargo test --workspace` is the suite; tests are not serial unless a task says so.
