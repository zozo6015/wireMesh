...
> **STATUS (2026-07-22): Tasks 1–4 DONE** (PR #16, unit-green) + the enrollment-client prerequisite (`2026-07-22-enrollment-client.md`). **The shipped code and the design spec's §0 Amendments are AUTHORITATIVE over the interface sketches below where they differ** — the Task-2 CRD structs (`crd.rs`: `Condition.type_` via `#[serde(rename="type")]`, the actual field sets) and the Task-4 builder signatures (`workloads.rs`: `gateway_deployment(gw, controller_sync, controller_enroll, ca_secret, token_secret)`, `relay_deployment(r, controller_sync, controller_enroll, ca_secret, token_secret)`, and a **native-sidecar** `bootstrap_init_container` — NOT the `token_secret`/`identity_secret`-only sketches at lines ~74/116) supersede the pre-implementation drafts here. **Next: Tasks 5–8 (reconcilers) + 9 (Helm/RBAC/kind e2e) + 10 (docs)**, built with the §0 amendments (admin-exec sidecar transport + `pods/exec` RBAC; PVC-backed gateway state; controller Service-exposure knob; gateway hostname fast-follow).
