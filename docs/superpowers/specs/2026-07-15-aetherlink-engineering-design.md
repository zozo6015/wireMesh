...
sequenceDiagram
    ...
    C-->>G: signed client cert + CA cert + gateway ID
...
erDiagram
    ...
    CERTIFICATE { string serial PK  string subject_kind  int subject_id  datetime not_after  datetime revoked_at }
...
- Backup story: `sqlite3 .backup` (or litestream) **plus the key directory** (§3); restore = replace both + restart; gateways reconnect with existing certs (C-7 — no re-enrollment after controller restart/restore).
...
The gateway reports its active backend to the controller (`fabricctl gateway list` shows it); behavior is identical by construction — the IR is the contract, and the same conformance packet-suite (§9) runs against both backends to prove parity, ICMP semantics included.
...
Single static Rust binary (`aetherlink-relay`). **Stateless means no session or flow state** (R-1, qualified): restart loses only in-flight sessions, which gateways re-establish automatically. The relay *does* persist its identity material — client cert + key, fabric CA cert, and the last-pushed revocation denylist (`/var/lib/aetherlink/`, 0600) — otherwise a relay restart during a controller outage would leave it unable to authenticate anyone, violating the spirit of fail-static.
...
2. Proto definitions for `Enrollment`, `Sync`, `Admin`.
