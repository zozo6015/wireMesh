...
| | |
|---|---|
| **Status** | Draft v0.4 (+ 2026-08-05 amendments: Non-Goals item 1, G-4a, X-7) |
| **Author** | Peter |
...
A standalone, cloud-agnostic, zero-trust L3/L4 network fabric written in Rust. It connects network *segments* (VPCs, VNets, VLANs, bare-metal subnets) rather than individual devices, using one gateway per segment. *(Since 2026-08-05 a single-host **client peer** is additionally in scope for hosts that must join directly — Non-Goals item 1; it does not change the segment model.)* No agents on workload machines — connectivity is a route change. Policy is default-deny with explicit L4 allow rules, distributed in real time from a central controller.
...
3. **Zero-trust by default**: 100% of inter-segment traffic is denied unless an explicit L4 allow rule exists. *(Client destinations are the one carved-out exception — see G-4a.)* Policy changes propagate controller → all gateways in **< 5 seconds** (p99).
...
1. **Device/user-level access (v1)** — ***PARTIALLY SUPERSEDED 2026-08-05; read the amendment below before citing this item.*** *Original text, kept for the record:* no per-laptop, per-user client. This is segment-to-segment routing. User-to-resource zero-trust (Twingate's core use case) is a possible future layer, but conflating the two would bloat v1. Twingate/Tailscale already serve that need.
   ...
   > - **The client address range is operator-chosen, exactly like any other segment CIDR — WireMesh does not pick one.** A client cannot take an address inside an existing segment's CIDR: the overlap check rejects it, and a more-specific `/32` route would hijack traffic to the real host at that address.
   >   **`100.64.0.0/10` is explicitly a POOR choice and must not be a default.** It was recommended and briefly ratified on 2026-08-05 as "overlaps nothing in the fabric" — the wrong test, since the collision is never with the fabric. Verified the same day on the intended first client: that Mac runs Twingate on `100.96.0.2` and a second tunnel on `100.116.0.0/16`, both inside `100.64.0.0/10`. It is RFC 6598 shared address space and **Tailscale and Twingate both claim it**, which makes it the worst available default for a VPN-adjacent product. Any hardcoded default is a latent collision; the operator chooses, and the preflight below is what makes that safe.
   >   **Conflict handling is required, not optional.** `100.64.0.0/10` is carrier-grade NAT space that real ISPs, mobile tethers and other VPNs hand out, so a client can find it already in use *locally* — a case the controller cannot see, since it only validates against registered fabric CIDRs. The failure mode is silently capturing the user's ISP-side traffic or losing their own connectivity, neither of which reads as a WireMesh problem. Two levels, deliberately distinguished:
   >   - **Provisioning-time preflight is MANDATORY.** The tool that generates a client profile must inspect that host's routing table and interface addresses, and **fail closed, naming the conflict**, rather than emit a profile it can see will collide.
   >   - **Connect-time preflight is required for production-grade, not for phase 1.** A profile generated on a clean network can still be carried onto a conflicting one later. Re-checking at every connect needs a wrapper that owns the toggle; the stock WireGuard app has no such hook. Phase 1 therefore ships provisioning-time only and **must document that moving networks requires re-provisioning** — it may not claim the conflict is handled continuously.
   > - **Roughly a dozen `client × segment` reachability pairs is the v1 ceiling** — *pairs*, not clients. One client is one segment (no per-peer address column exists), so each segment a client may reach costs its own policy block, and `MAX_RULES = 256` is checked after port explosion. One client permitted everywhere costs as much as several tightly scoped ones. Beyond that needs a per-peer address model.
   >
...
3. As a **security engineer**, I want all inter-segment traffic denied by default (client destinations excepted, per G-4a) with explicit allows per (src CIDR, dst CIDR, port range, protocol), so that I can prove least-privilege segmentation.
...
| C-4 | **Clients: see X-7.** **Policy distribution.** ACL rule sets are versioned; gateways ack the applied version. | `fabricctl policy status` shows the policy version applied per gateway; a stuck gateway is visibly lagging. |
...
| G-3 | **Clients: see X-7.** **Full-mesh tunnel establishment** to all peer gateways, with per-pair automatic relay fallback. | With UDP blocked between two gateways, the pair converges to relay transport within 30s without operator action, and reverts to direct when possible. |
...
| G-4a | **Client carve-out (added 2026-08-05 with the Non-Goals item 1 amendment).** Enforcement is *ingress-on-tun only*, so for a **client peer** (Non-Goals item 1): `client → segment` **is** policed, at the receiving gateway; `segment → client` is policed by **nothing**, because the sending gateway's tun egress never drops and the client runs no enforcer; and `client → client` is policed by nothing **in either direction**, because a direct client-to-client tunnel has no gateway in the path at all. A **known, accepted exception to G-4's default-deny guarantee**, scoped to client destinations — segment-to-segment default-deny is unaffected, as is backend parity, since neither backend enforces these directions. | The exception is documented wherever clients are, and the client's own host firewall is named as the operator's responsibility. **Clients MUST NOT peer with other clients**: until hub-and-spoke projection ships (clients peer with gateways only), a client may not be a Sync peer, because the punch broker would otherwise establish exactly the unenforced client↔client tunnel described. **Egress-side enforcement on gateways for client-destined traffic is required before a client may be called production-grade**; until it ships, no doc, release note, or UI may imply a client is protected inbound. Closing this restores plain G-4 and must keep eBPF/nftables parity. |
...
| G-7 | **Clients: see X-7.** **Graceful drain/decommission.** | `fabricctl gateway drain <name>` withdraws routes from peers before teardown; peers stop routing to it within 5s. |
...
| X-2 | **Clients: see X-7.** **Observability:** Prometheus metrics on all components (tunnel/handshake state, RTT per peer, bytes per peer, policy version, denied-packet counters, relay throughput); structured logs (JSON). | A reference Grafana dashboard ships in the repo. |
...
| X-7 | **Client peer — scope limits (added 2026-08-05 with the Non-Goals item 1 amendment).** A client is an in-scope component with materially narrower guarantees than a gateway, and this row is the single place they are stated so no other requirement is silently over-claimed. A **phase-1 client does NOT satisfy**: **C-9** (revocation reaching all peers in 5s — a static profile has no Sync stream, so a drained/revoked peer is not propagated to it), **G-7** (drain within 5s — same reason: it keeps a drained gateway's peer entry until re-provisioned), **G-3** (per-pair relay fallback — no relay participation, so it must reach a gateway with a routable endpoint; a client behind CGNAT has no path), **C-4** (lag visibility — it never reports `applied_version`, so it reads as permanently stuck at 0 and would fire roster-lag alerting forever), **X-2** (Prometheus metrics — it exports none), and **X-6** (version skew — it is outside the skew contract). It is also subject to the **G-4a** enforcement carve-out. | Every limitation above is documented wherever clients are provisioned, and none is described as a defect or a bug — they are the phase-1 contract. `fabricctl` must not let a client's `applied_version = 0` raise a lag alert. **A client may not be described as production-grade until C-9, G-7, C-4 and G-4a are met**; G-3 and X-2 may remain out of scope for a static client by explicit decision, recorded here if so. |
...
- **User/device access layer** — *identity* for the now-in-scope client peer: user authentication, device posture, per-user policy. The *connectivity* half shipped under Non-Goals item 1's amendment; this is the remainder that would converge on the full Twingate use case.
...
| Scope gravity toward device-level access ("just add a client") | High | High | **Realised in part, 2026-08-05.** The *connectivity* half was taken in (Non-Goals item 1 amendment) with a hard ceiling, a G-4a carve-out and no identity layer — after segment routing was proven, as the mitigation intended. The *identity* half (user auth, device posture, per-user policy) stays P2 and is the line to hold. |
